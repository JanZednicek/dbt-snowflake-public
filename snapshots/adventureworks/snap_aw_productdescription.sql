...
-----------------------------------------------------------------------------
  ...
  About this table:
    - Table:    snap_aw_productdescription.sql
    - Schema:   staging | db based on your --target (profiles.yml)
    - Source:   adventureworks database (sqlservercentral.com) - table productdescription.sql | Data extracted using fivetran
...
{% snapshot snap_aw_productdescription %}
    ...
    select * from {{ source('ADVENTUREWORKS', 'PRODUCTDESCRIPTION') }}
