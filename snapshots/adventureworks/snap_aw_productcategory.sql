...
-----------------------------------------------------------------------------
  ...
  About this table:
    - Table:    snap_aw_productcategory.sql
    - Schema:   staging | db based on your --target (profiles.yml)
    - Source:   adventureworks database (sqlservercentral.com) - table productcategory.sql | Data extracted using fivetran
...
{% snapshot snap_aw_productcategory %}
    ...
    select * from {{ source('ADVENTUREWORKS', 'PRODUCTCATEGORY') }}
